	...
	<view class="page">
		<scroll-view class="scrollList" scroll-y :scroll-into-view="scrollViewId" :style="{height:winHeight+'px'}">
			<view class="search-bar">
				<view class="search-bar-form">
					<view class="search-bar-box">
						...
						<input confirm-type="search" class="search-bar-input" placeholder="请输入搜索关键词" placeholder-class="phcolor" :value="inputVal" @input="inputTyping" @confirm="searchCity" />
			...
			<view class="tui-list search-result" v-if="inputShowed">
				<view class="tui-list-cell" hover-class="tui-list-cell-hover" v-for="(item,index) in searchResult" :key="index" :data-name="item" :hover-stay-time='150' @click="distsubmenu(item)">
					<view class="tui-list-cell-navigate">
                        <u-avatar :src="item.headimg" class="img" ></u-avatar>
						<view class="content">
			...
			<view v-if="!inputShowed">
				<view class="tui-list city-list">
					<block v-for="(list,index) in lists" :key="index" v-if="list.data[0]">
						...
						<view class="tui-list-cell" hover-class="tui-list-cell-hover" v-for="(item,index2) in list.data" :key="index2" :data-name="item.name" :hover-stay-time='150'  @click="distsubmenu(item)">
        ...
        <u-action-sheet :list="shoplist"  @click="changeshop" v-model="shopshow"></u-action-sheet>
	</view>
</template>
...
<script>
	export default {
		...
		onLoad(e) {
            this.currshop={'shopname':'佳丰种业','shopcode':'5ebd18fd18ac0','shopnote':'应县'};
            ...
            uni.request({
                url: 'http://www.jfseed.com.cn/appapi/jfseed_admin/dist/dist_api.php',
                data: {
                    'action':"get_distributor_list",
                    'shopcode':this.currshop.shopcode
                },
                success: (res) => {
                   // console.log(res.data);
                    if (res.data.length<1){
                        ...
                        uni.setNavigationBarTitle({
                        	title: this.currshop.shopnote+"分销商("+totaldist+"人)"
                        });
                    }
                }
            })
                    ...
                    uni.request({
                        ...
                        success: (res) => {
                            if (res.data.length<1){
                                ...
                                uni.setNavigationBarTitle({
                                	title: this.currshop.shopnote+"分销商(0人)"
                                });
                                this.isloading=false;
                            }else{
                                this.lists = res.data
        ...
        onNavigationBarButtonTap(e){
            if(e.index==0){
                uni.navigateTo({
                    url:"./distributorAdd?shop_info="+encodeURIComponent(JSON.stringify(this.currshop))
                })
            }
            if(e.index==1){
		...
		methods: {
            changeshop(e){
                this.currshop=this.shoplist[e];
                console.log(this.currshop)
            },
            distsubmenu(item){
                uni.navigateTo({
                    url:"./distributorSubmenu?shop_info="+encodeURIComponent(JSON.stringify(this.currshop))+"&dist_info="+encodeURIComponent(JSON.stringify(item))
                })  
            },
			clearInput() {
				this.inputVal = "";
